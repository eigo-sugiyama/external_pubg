...
void render()
{
	...
	for (int i = 0; i < HongEntities.size(); ++i)
	{
		...
		DrawString(Width / 2, 0, D3DCOLOR_ARGB(255, 255, 0, 0), pFont, ("������[" + to_string(SpectateCount) + "]").c_str());
			...
			if (GetAsyncKeyState(VK_RBUTTON) && Settings::aimbot)
			{
				if (IsVisible)
				{
					...
					if (distance < 450 && Dist < FOV)
					{
						...
						//float BulletSpeed = 875.0f; // M416 ź�� = 880.f
...
void SetupWindow()
{
	...
	{
		std::cout << u8"������ ����Ǿ����ϴ�." << GetLastError() << std::endl;
		Sleep(3000);
		Cleanup();
		ExitProcess(0);
	}
